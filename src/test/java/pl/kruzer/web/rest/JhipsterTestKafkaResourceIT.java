...
import pl.kruzer.config.KafkaProperties;
...
class JhipsterTestKafkaResourceIT {
    ...
    void setup() {
        ...
        JhipsterTestKafkaResource kafkaResource = new JhipsterTestKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/jhipster-test-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/jhipster-test-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
